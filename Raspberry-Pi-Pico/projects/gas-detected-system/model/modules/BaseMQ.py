...
class BaseMQ(Adc):
    ...
    def __init__(self, channel=0, pinHeater=-1, boardResistance=10, baseVoltage=5.0, measuringStrategy=STRATEGY_ACCURATE):
    ...
    def calibrate(self, ro=-1):
        if ro == -1:
            ...
            for i in range(0, MQ_SAMPLE_TIMES + 1):
                ...
                temp_adc = self.getValue()
                ro += self.__calculateResistance__(temp_adc)
    ...
    def __readRs__(self):
        if self.measuringStrategy == STRATEGY_ACCURATE:
            rs = 0
            for i in range(0, MQ_SAMPLE_TIMES + 1):
                rs += self.__calculateResistance__(self.getValue())
                utime.sleep_ms(10)
